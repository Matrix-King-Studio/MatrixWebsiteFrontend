<template>
  <div class="wrapper">
    ...
    <div class="nav" ref="nav" style="opacity: 0;">
      ...
      <div class="ul cp">
        <div class="li" :class="liIndex==index?'acitve':''" v-for="(item,index) in navLi"
        @click="liCli(index)" >
          {{item.name}}
        </div>
      </div>
...
export default {
  ...
  data(){
    return{
      ...
      topText:
      `Matrix工作室是由天津科技大学人工智能学院的一群编程爱好者组成的，大家志同道合，相互鞭策，一起努力变得更优秀。Matrix工作室分为四个组，分别是AI组、IOS组、前端组、后端组。同时，每个组都有各组组长精心设计好的学习路线和学习资料，每隔一断时间都会调整。
